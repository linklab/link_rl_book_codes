...
LEFT_PAD = '  '
LOG_FMT = logging.Formatter('%(levelname)s [%(filename)s:%(lineno)d] %(message)s %Y-%m-%d %H:%M:%S')
...
def after_action_state(state, action):
    ...
    board, mark = state
    nboard = list(board[:])
    nboard[action] = to_code(mark)
    return tuple(nboard), next_mark(mark)
...
class TicTacToeEnv(gym.Env):
    ...
    def __init__(self, alpha=0.02, show_number=False):
        ...
        self.observation_space = spaces.Discrete(NUM_LOCATIONS)
        self.alpha = alpha
    ...
    def _show_board(self, showfn):
        ...
        for j in range(0, 9, 3):
            def mark(i):
                return to_mark(self.board[i]) if not self.show_number or self.board[i] != 0 else str(i + 1)
    ...
    def _show_result(self, showfn, mark, reward):
        ...
        if status == 0:
            ...
            msg = "Winner is '{}'!".format(to_mark(status))
