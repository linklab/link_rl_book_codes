        ...
        if not expected:
            ...
            for action_ in env.action_space.ACTIONS:
                if action_ in best_actions:
                    next_q += ((1.0 - EPSILON) / len(best_actions) + EPSILON / NUM_ACTIONS) * q_value[next_state[0], next_state[1], action_]
                else:
...
def cumulative_rewards_for_step_size(env):
    ...
    labels = [
        '기대값 기반 SARSA (1000 에피소드)', 'SARSA (1000 에피소드)', 'Q-러닝 (1000 에피소드)',
        '기대값 기반 SARSA (100 에피소드)', 'SARSA (100 에피소드)', 'Q-러닝 (100 에피소드)'
    ]
